/*
Municipality_code initially starting by 2A are now starting by 2999
Municipality_code initially starting by 2B are now starting by 2000
...
SELECT
  *
, REGEXP_REPLACE(municipality_code, 'B', '000') AS municipality_codeB
FROM `prello-lewagon.prello.notary_real_estate_sales`
),

create_department AS (
SELECT
    *
, LEFT(municipality_code,2) AS department     -- create department column
, REGEXP_REPLACE(municipality_codeB, 'A','999') AS municipality_codeAB
FROM muni_code
),
...
, department
, premise_type
, SUM(ROUND(CAST(municipality_codeAB AS FLOAT64),0)) AS post_code
...
, COUNT(*) AS nb
FROM create_department WHERE department < '97'  -- remove overseas department
